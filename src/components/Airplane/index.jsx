/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
Command: npx gltfjsx@6.1.4 public/models/airplane/untitled.glb
*/

import React, { useRef } from "react";
import { useGLTF } from "@react-three/drei";
import { useFrame } from "@react-three/fiber";

export function Airplane(props) {
	const { nodes, materials } = useGLTF("./models/airplane/model.glb");

	const helix = useRef();

	useFrame((_state, delta) => {
		helix.current.rotation.x += delta * 8; // HELIX SPEED = 6
	});

	return (
		<group
			{...props}
			dispose={null}
		>
			<mesh
				geometry={nodes.PUSHILIN_Plane_Circle000.geometry}
				material={materials.plane}
				position={[0.01, 0.03, 0]}
				rotation={[0, 0, 0.05]}
			>
				{/* <meshStandardMaterial color={"#ffffff"} /> */}
			</mesh>
			<mesh
				ref={helix}
				geometry={nodes.PUSHILIN_Plane_Circle001.geometry}
				material={materials.plane}
				position={[1.06, 0.31, 0]}
				rotation={[1.33, 0.25, -0.15]}
			>
				{/* <meshStandardMaterial color={"#ffffff"} /> */}
			</mesh>
		</group>
	);
}

useGLTF.preload("./models/airplane/model.glb");
